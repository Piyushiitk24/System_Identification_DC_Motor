...
The static map rests on a single sweep, validated by two independent checks rather than by full repeat sweeps. No \texttt{static\_sweep\_run02} or \texttt{run03} were collected; the reproducibility argument rests on this single sweep, the two-point spot-check below, and the implicit cross-validation provided by the Phase~2.1 step-response steady-state endpoints. First, a two-point spot-check at PWM~=~$\pm 255$, repeated morning and evening of the same day, isolated where any drift occurs (\tref{tab:drift}).
...
\chaptersources{\texttt{data/raw/static\_sweep\_20260506\_run1.csv} (sweep, $K$ fits, V/RPM ratios, full-duty droop) \textperiodcentered{} \texttt{data/raw/deadzone\_manual\_run01.csv} (breakaway/dropout, V/RPM at threshold) \textperiodcentered{} \texttt{data/raw/spot\_check\_max\_pwm\_2026-05-06\_evening.csv} (thermal drift) \textperiodcentered{} \texttt{notebooks/01\_static\_map.ipynb} (figures 01--04, gain computation) \textperiodcentered{} status-doc \S4.1 (SS hysteresis at PWM~160, cross-val) \textperiodcentered{} \texttt{thesis\_notes/log.md} 2026-05-06/07 (Phase 1.1/1.2 interpretation).}
