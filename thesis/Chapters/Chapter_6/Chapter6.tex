...
$K_{ss}$ rises ${\approx}4$--$7\,\%$ and $\tau$ falls ${\approx}3$--$5\,\%$ over the session, with forward drift larger than reverse. The direction of $K_{ss}$ drift (higher steady speed for the same PWM at session end) is consistent with the L298N saturation drop falling as junction temperature rises---the driver delivers slightly more motor voltage as it warms, so the same PWM gives slightly more RPM. The $\tau$ contraction (faster transient) is the same effect on a different axis: more delivered voltage means more current available to accelerate the rotor. The drift is small enough that the closed-loop comparison itself---which runs each pair within seconds of each other---is unaffected, but it is large enough that \emph{between}-session reuse of parameters would still be suspect, confirming once more \sref{sec:xsession-val}'s recalibrate-in-situ requirement.
