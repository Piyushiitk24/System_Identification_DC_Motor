...
\paragraph{Controllers.} Both controllers are discrete PI with anti-windup, $\pm 255$\,PWM clamp, and 30-counts-per-sample output slew limit; identical control-loop arithmetic in firmware (\texttt{src\_closedloop/main.cpp}) and in the Python simulator (\texttt{motor\_id/controllers.py}).
\begin{itemize}
...
\end{itemize}
The same \texttt{compute\_imc\_gains}$(K, \tau, T_d, \lambda)$ function tunes both controllers; neither was hand-adjusted on the bench. Gains were frozen from the in-session calibration before any closed-loop trial began and persisted to \texttt{models/closed\_loop\_gains\_2026-05-27.json}.
...
\begin{enumerate}
  ...
  \item \textbf{On-laptop refit}\,---\,FOPDT fits per condition; Model~A globally; IMC gains and FF table built and persisted; uploaded to firmware via the \texttt{GAINS\_BASE}/\texttt{GAINS\_CASC}/\texttt{FF\_FWD}/\texttt{FF\_REV} commands. \textbf{Gains frozen from this point.}
...
\paragraph{Metrics (pre-registered).} Per trial: \texttt{rmse\_full} (post-warm-up window), \texttt{rmse\_deadzone} (samples where $|{\text{ref}}| \le 80$\,rpm), \texttt{rmse\_transient} ($5\tau$ after each setpoint change), settling time, overshoot, steady-state error, RMS PWM. The headline metric is \texttt{rmse\_full}. Headline statistic is the per-pair difference $\mathrm{RMSE}(\text{BASE}) - \mathrm{RMSE}(\text{CASC})$.
...
\bottomrule
\end{tabular}
\caption{Pre-registered paired RMSE difference per profile, $\text{BASE} - \text{CASC}$. Positive $=$ cascade wins. ``Sim pred.'' is the simulated $\text{BASE} - \text{CASC}$ from \texttt{notebooks/07a\_closed\_loop\_sim.ipynb}, run before bench day with Model~C as truth plant.}
\label{tab:cl-paired}
\end{table}
...
$K_{ss}$ rises ${\approx}4$--$7\,\%$ and $\tau$ falls ${\approx}3$--$5\,\%$ over the session, with forward drift larger than reverse. The direction of the drift (higher steady speed and faster transient for the same PWM at session end) is consistent with a reduction in effective drive loss or low-speed friction as the rig warms, mirroring the morning-vs-evening spot-check of \sref{sec:static-repro} where motor voltage was measured and showed the same pattern. However, Phase~3 itself did not log motor voltage or L298N temperature, so the drift cannot be uniquely localised to the driver in this session---the same observation is consistent with rotor/bearing/lubricant warming. The drift is small enough that the closed-loop comparison itself---which runs each pair within seconds of each other---is unaffected, but it is large enough that \emph{between}-session reuse of parameters would still be suspect, confirming once more \sref{sec:xsession-val}'s recalibrate-in-situ requirement.
...
\chaptersources{\texttt{data/processed/closed\_loop\_metrics\_per\_trial.csv} (per-trial metrics, $n=64$) \textperiodcentered{} \texttt{data/processed/closed\_loop\_summary.json} (pre-registered tests, headline numbers) \textperiodcentered{} \texttt{data/processed/closed\_loop\_drift\_check.csv} (start-vs.-end drift) \textperiodcentered{} \texttt{models/calibration\_2026-05-27.json} (in-session refit of Models A and C) \textperiodcentered{} \texttt{models/closed\_loop\_gains\_2026-05-27.json} (frozen PI gains and FF table) \textperiodcentered{} \texttt{data/processed/phase3\_preregistration.json} (committed pre-registration, SHA-256 in \texttt{thesis\_notes/log.md}) \textperiodcentered{} \texttt{notebooks/07a\_closed\_loop\_sim.ipynb} (sim predictions, pre-registration freeze) \textperiodcentered{} \texttt{notebooks/07b\_closed\_loop\_results.ipynb} (per-trial metrics, paired tests, figures) \textperiodcentered{} figures~\ref{fig:cl-paired-diff} (paired differences with CIs), \ref{fig:cl-trajectories} (representative trajectories) \textperiodcentered{} \texttt{src\_closedloop/main.cpp}, \texttt{motor\_id/\{cascade\_sim,controllers,model\_io,metrics,calibration\}.py} (controller \& analysis implementations).}
