...
Demonstrating the model's value in **closed loop** — showing that a cascade-aware controller outperforms one designed against a naive global linear model — is the natural culmination but is deferred: it is scoped here as a concrete, actionable plan (Chapter 6) rather than executed, because it requires a further bench session with in-session recalibration that the identification work itself shows to be necessary.
