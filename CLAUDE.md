...
The README and `manual.md` describe the original Phase 0-1 manual workflow and are no longer the latest state. The current state is Phase 2 gap-fill after Phase 2.2 LOOCV validation, plus a quantified A/B/C model-selection ladder added on 2026-05-25 in `notebooks/06_model_comparison.ipynb` (Phase 2.1 grid headline + full-envelope extension across both sessions; fitted A/B/C parameters persisted in `models/model_{A,B,C}.json`). The source of truth for the latest modeling conclusions is `thesis_notes/log.md` plus the processed CSVs in `data/processed/` and the notebooks, especially `notebooks/05_gap_fill.ipynb` and `06_model_comparison.ipynb`.

For thesis-facing work, the source of truth for prose is the chapter drafts in `thesis_notes/draft_ch*.md` (the standalone model-selection draft is `thesis_notes/draft_model_selection.md`, integrated as §4.6 of `thesis/Chapters/Chapter_4/Chapter4.tex`), and the cross-checked snapshot of every reported number is `thesis_notes/repo_status_2026-05-21.md` (the "status-doc"; predates the model-ladder, which is captured in `log.md` 2026-05-25 instead). The LaTeX is built: `thesis/main.pdf` is current at ~79 pages with §4.6 included. See *Thesis writing layer* below before editing either.
...
Notebooks are numbered by phase: `01_static_map.ipynb` (Phase 1.1), `03_step_responses.ipynb` (Phase 2.1 FOPDT), `04_validation.ipynb` (Phase 2.2 LOOCV), `05_gap_fill.ipynb` (Phase 2 gap-fill, session drift, refined LOOCV), and `06_model_comparison.ipynb` (in-sample A/B/C structural ladder on the Phase 2.1 grid plus a full-envelope extension that adds the gap-fill conditions). Notebook 06's structural comparison is distinct from notebooks 04/05's LOOCV, which is the *generalisation* test — keep the two questions separate when interpreting their numbers. When adding cells, do explicit schema checks before fitting or plotting, and keep cells runnable after a kernel restart (re-import `Path`, `glob`, `pandas`, `numpy` locally in debugging cells the user is likely to rerun in isolation).
...
- **LaTeX build** (`thesis/`): the IITK template (working copy; pristine original is in `IITK Thesis Template (LaTeX Code File)/`). Compile `thesis/main.tex` with `latexmk -pdf` (run from inside `thesis/`). The preamble is trimmed for a local BasicTeX (2025basic) install — the note in `main.tex` lists the packages removed and to restore for a full TeX Live / Overleaf build. Figures live in `thesis/Pictures/` (mirrors `figures/`). The current build (`thesis/main.pdf`, ~79 pages, last rebuilt 2026-05-25) reflects Chapters 1–7 with the integrated §4.6 *Model selection: the A/B/C ladder, quantified* — Tables 4.3–4.4 and Figures 4.6–4.8 (= generated `figures/20`–`22`). Copy newly-generated figures from `figures/` to `thesis/Pictures/` before rebuilding when the LaTeX references them by bare name (`\graphicspath{{Pictures/}}` is set in `main.tex`).
