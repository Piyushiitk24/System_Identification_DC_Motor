...
For thesis-facing work, the source of truth for prose is the chapter drafts in `thesis_notes/draft_ch*.md` (the standalone model-selection draft is `thesis_notes/draft_model_selection.md`, integrated as §4.6 of `thesis/Chapters/Chapter_4/Chapter4.tex`; Chapter 6 is the Phase 3 closed-loop *results* chapter, rewritten 2026-05-27), and the cross-checked snapshot of every reported number is `thesis_notes/repo_status_2026-05-27.md` (the "status-doc"; supersedes `repo_status_2026-05-21.md`). The LaTeX is built: `thesis/main.pdf` is 90 pages (last rebuild 2026-06-10, 0 undefined refs) and includes the rewritten Phase 3 Chapter 6 (Tables 6.1–6.2, Figures 6.1–6.3 = generated `figures/23`–`25`). Figure 6.3 is the P2 zero-crossing zoom that makes the FF-discontinuity mechanism visible. The pre-registration for Phase 3 is at `data/processed/phase3_preregistration.json` (SHA-256 `da94c83d...02707669`, locked 2026-05-26 before bench day); raw per-pair data at `data/processed/closed_loop_pair_differences.csv`. See *Thesis writing layer* below before editing either.
...
- **LaTeX build** (`thesis/`): the IITK template (working copy; pristine original is in `IITK Thesis Template (LaTeX Code File)/`). Compile `thesis/main.tex` with `latexmk -pdf` (run from inside `thesis/`). The preamble is trimmed for a local BasicTeX (2025basic) install — the note in `main.tex` lists the packages removed and to restore for a full TeX Live / Overleaf build. Figures live in `thesis/Pictures/` (mirrors `figures/`). The current build (`thesis/main.pdf`, 90 pages, last rebuilt 2026-06-10, 0 undefined refs) reflects Chapters 1–7 with §4.6 *Model selection: the A/B/C ladder, quantified* (Tables 4.3–4.4, Figures 4.6–4.8 = `figures/20`–`22`) and the Phase 3 Chapter 6 results (Tables 6.1–6.2, Figures 6.1–6.3 = `figures/23`–`25`). Copy newly-generated figures from `figures/` to `thesis/Pictures/` before rebuilding when the LaTeX references them by bare name (`\graphicspath{{Pictures/}}` is set in `main.tex`).

  **Standalone-final-report invariants (added 2026-06-10).** The thesis `.tex` prose is now a self-contained final document, polished away from its draft-rooted origins. Two rules hold and must be preserved on any future edit: (1) **no em dashes** (`---`) in rendered prose — use a comma, colon, or parentheses instead (en-dash `--` ranges like `1--5` rpm are fine); (2) **no repo/code identifiers** in the prose — no filenames, CSV/notebook/`.cpp`/`.py` paths, serial-command names, struct/function names, or telemetry column names rendered in `\texttt{}`/`\verb`/`\mathtt`. Describe such things in words (e.g. "the telemetry columns" not the literal header). The per-chapter `\chaptersources{...}` repo-provenance footers were emptied/removed (they were already a no-op macro). The matching `thesis_notes/draft_ch*.md` working drafts were **not** re-polished in this pass, so the `.tex` is now ahead of them on prose style; treat the `.tex` as the prose source of truth for em-dash/standalone hygiene.
...
### Figure-baked text must be standalone too (added 2026-06-10)

The standalone-report invariants apply to text *rendered into figure images* (titles, axis labels, legends, annotations), not just the LaTeX prose: no draft jargon or repo/code identifiers baked into a PNG. In the 2026-06-10 pass, `notebooks/05_gap_fill.ipynb` figs 14/15/16/18 were cleaned ("piggy run"/"Piggyback"/"piggyback" → "re-run"/"Cross-session re-run"; "P21" → "Phase 2.1") by editing *display strings only* (never variable names like `PIGGYBACK_ORDER`), re-executing the notebook, and mirroring the regenerated PNGs to `thesis/Pictures/`. Note `notebooks/05` re-execution is **not** bit-deterministic: the `scipy` FOPDT refits jitter at ~1e-9, harmlessly rewriting `data/processed/phase2gapfill_*.csv` — `git checkout`-restore those CSVs after a cosmetic re-run so locked numbers stay byte-stable (the thesis quotes them to 0–2 decimals, so nothing actually moves). Figure *filenames* (e.g. `14_…_piggyback_overlay.png`) are kept as-is; they never render in the PDF.
